...
bool test_buffer()
{
    if ((num_of_clk_passed >= BUFFER_SIZE + USER_HOLDOFF + ALIGNMENT_DELAY + 1) && (num_of_clk_passed <= 2*BUFFER_SIZE + USER_HOLDOFF + ALIGNMENT_DELAY)) {
	    ...
	    cout << "Data in the circular buffer is not correct for buffer items at " << (fmod(num_of_clk_passed , (BUFFER_SIZE+1))) << " at clock cycle " << num_of_clk_passed << endl;
	    return false;
    	}    
    }
    ...
    if (num_of_clk_passed <= BUFFER_SIZE + ALIGNMENT_DELAY) {  
	    ...
	    cout << "Memory is not yet fully initialized. Scope could not stop recording at clock cycle " << num_of_clk_passed << endl;
	    return false;
	}
    }
...
int main(int argc, char** argv)
{
    ...
    unsigned int counter = 1;  // this is the test signal we are going to record into circular buffer
	...
	cout << "assert_trigger_now = " << assert_trigger_now << endl;	
